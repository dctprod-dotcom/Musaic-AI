      ...
      return (
        <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 text-center">
          <div className="space-y-6 max-w-md">
            ...
            <h1 className="text-2xl font-black uppercase text-white">Something went wrong</h1>
            <p className="text-gray-400 text-sm">{this.state.error?.message}</p>
            <button onClick={() => window.location.reload()} className="px-8 py-3 bg-turquoise text-black font-bold uppercase rounded-xl">Reload</button>
          </div>
        </div>
      );
    }
...
// ─── Dashboard — fits viewport, no scroll needed ─────────
function Dashboard({ t, onNavigate, user, onSignUp }: { t: (k: string) => string; onNavigate: (id: ModuleId) => void; user: any; onSignUp: () => void }) {
  const modules = [
    { id: 'smart-link' as ModuleId, title: 'SMART LINK', desc: 'All your links in one page', icon: Globe, color: 'text-emerald-400', bg: 'bg-emerald-500/10', ring: 'ring-emerald-500/20' },
    { id: 'epk' as ModuleId, title: 'PRESS KIT', desc: 'Bio & press releases', icon: FileText, color: 'text-pink-500', bg: 'bg-pink-500/10', ring: 'ring-pink-500/20' },
    { id: 'artwork' as ModuleId, title: 'ARTWORK', desc: 'AI cover art & visuals', icon: ImageIcon, color: 'text-turquoise', bg: 'bg-turquoise/10', ring: 'ring-turquoise/20' },
    { id: 'video' as ModuleId, title: 'VIDEO', desc: 'AI video for socials', icon: Video, color: 'text-purple-neon', bg: 'bg-purple-neon/10', ring: 'ring-purple-neon/20' },
  ];
      ...
      <div className="mb-3 lg:mb-4 flex items-end justify-between">
        <div>
          ...
          <p className="text-white/40 text-[10px] font-semibold uppercase tracking-[0.3em] mt-0.5">
            {user ? 'Choose your module' : 'Explore — sign up to create'}
          </p>
        </div>
        {user && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gold/10 border border-gold/20">
            ...
            <span className="text-[9px] text-gold/60 font-semibold">tokens</span>
          </div>
        )}
      </div>
      ...
      {!user && (
        <div className="mb-3 p-3 rounded-2xl bg-turquoise/5 border border-turquoise/20 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-white/70">Sign up → <span className="text-turquoise font-bold">50 free tokens</span></p>
          </div>
...
function AuthGate({ onSignUp, moduleName }: { onSignUp: () => void; moduleName: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center space-y-6">
      ...
      <h2 className="text-2xl font-black uppercase tracking-tight text-white">Sign in to create</h2>
      <p className="text-white/40 text-sm max-w-md">You can explore {moduleName} freely, but you'll need an account to generate and save content.</p>
...
function App() {
      ...
      if (u) {
        try {
          ...
          if (userDocSnap.exists()) {
            ...
            userData = { email: u.email, displayName: u.displayName || '', avatar: u.photoURL || '', points: 50, isPro: false, createdAt: serverTimestamp() };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') setAuthError('Account exists. Sign in instead.');
      ...
      else if (error.code === 'auth/weak-password') setAuthError('Password too weak (6+ chars).');
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-white/40 text-[10px] font-medium tracking-wider uppercase mt-1">{isSignUp ? '50 free tokens on signup' : t('auth.signIn')}</p>
              </div>
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                {resetSent && <div className="bg-turquoise/10 border border-turquoise/30 p-3 rounded-xl text-turquoise text-xs">Check your inbox to reset your password.</div>}
                <div className="space-y-3">
                  <div className="relative group">
                    ...
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15" placeholder="Email address" />
                  </div>
                  <div className="relative group">
                    ...
                    <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15" placeholder="Password" />
                  </div>
              ...
              <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
                ...
                <div className="space-y-1">
                  ...
                  {([
                    ...
                    { icon: Mail, label: t('nav.contact'), module: 'contact' },
                  ...
                  {user ? (
                    <>
                      <button onClick={handleLogout} className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 transition-all group text-left">
                        <LogOut className="w-5 h-5 text-white/40" /><span className="text-xs font-bold uppercase tracking-widest text-white/60 group-hover:text-white">{t('nav.signOut')}</span>
                      </button>
                      <button className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-red-500/10 transition-all group text-left mt-4">
                        <Trash2 className="w-5 h-5 text-red-500/60 group-hover:text-red-500" /><span className="text-xs font-bold uppercase tracking-widest text-red-500/60 group-hover:text-red-500">{t('nav.deleteAccount')}</span>
                      </button>
                    </>
                  ) : (
                    <>
