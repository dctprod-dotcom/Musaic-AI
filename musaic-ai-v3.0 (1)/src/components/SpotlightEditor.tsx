...
export function SpotlightEditor({ user, t, onBack, generatedAssets }: { user: any, t: (key: string) => string, onBack: () => void, generatedAssets?: any }) {
  ...
  // Carousel Auto-Play
  useEffect(() => {
    if (formData.selectedAssets?.length > 1) {
  ...
  const checkSlug = async (slug: string) => {
    ...
    const res = await fetch(`/api/spotlight/check-slug?slug=${slug}`);
        ...
        if (type === 'profile') {
          ...
          try {
            ...
            if (res.ok) {
              const data = await res.json();
              // Refresh assets
              const libRes = await fetch('/api/library/list');
              const libData = await libRes.json();
              setAssets(libData);

              // If API returns the asset ID, use it. Otherwise try to find it in the new list.
              const newAssetId = data.asset?.id || libData.find((a: Asset) => a.title === (file.name || 'Background Upload'))?.id;
