...
import {
  ...
  Send, ChevronDown, Check, Crown, ArrowLeft, Clock,
  Link2, Copy, ExternalLink, Download, Wand2
} from 'lucide-react';
import {
...
const SITE_DOMAIN = "musaicaistudio.com";
...
// ── Sidebar nav — 'Artist Hub' always EN as requested ─────
const NAV_ITEMS = [
...
// ── Token deduction (with admin bypass) ───────────────────
async function deductTokens(userId: string, cost: number, isAdmin: boolean): Promise<boolean> {
  ...
  return (
    <div className="h-full flex flex-col p-4 lg:p-6 max-w-5xl mx-auto">
      ...
      <div className="mt-3 grid grid-cols-3 gap-3">
        <button onClick={() => onNavigate('release-hub')} className="flex items-center gap-2 p-3 glass-card rounded-xl hover:bg-white/5 transition-all group">
        ...
        <button onClick={() => onNavigate('tutorials')} className="flex items-center gap-2 p-3 glass-card rounded-xl hover:bg-white/5 transition-all group">
        ...
        <button onClick={() => onNavigate(user ? 'my-account' : 'membership')} className="flex items-center gap-2 p-3 glass-card rounded-xl hover:bg-white/5 transition-all group">
...
// ═══════════════════════════════════════════════════════════
// ─── SMART LINK MODULE (Linktree-style, short URL) ───────
// ═══════════════════════════════════════════════════════════
function SmartLinkModule({ t, user, isAdmin, onBack, onGenerate }: {
  t: (k: string) => string; user: any; isAdmin: boolean;
  onBack: () => void; onGenerate: () => Promise<boolean>;
}) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Store in Firestore collection 'links'
      ...
      setGeneratedUrl(`${SITE_DOMAIN}/l/${slug}`);
    } catch (err) {
      ...
      {generatedUrl ? (
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-3xl p-6 lg:p-8 space-y-5">
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('smartlink.releaseTitle')}</label>
            ...
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('smartlink.artistName')}</label>
              <input type="text" value={artistName} onChange={e => setArtistName(e.target.value)}
                className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15"
                placeholder={t('smartlink.artistNamePlaceholder')} />
            </div>
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('smartlink.coverUrl')}</label>
          ...
          <div className="glass-card rounded-3xl p-6 lg:p-8 space-y-4">
            <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">{t('smartlink.platformLinks')}</p>
            {links.map((link, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="text-xs font-bold text-white/50 w-28 flex-shrink-0">{link.platform}</span>
                <input type="url" value={link.url} onChange={e => updateLink(i, e.target.value)}
                  className="flex-1 bg-black/40 border border-white/10 rounded-xl py-2.5 px-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15"
                  placeholder={`https://${link.platform.toLowerCase().replace(/\s/g, '')}.com/...`} />
              </div>
            ))}
          </div>
...
// ═══════════════════════════════════════════════════════════
// ─── ARTWORK MODULE (Imagen 3 simulation) ────────────────
// ═══════════════════════════════════════════════════════════
function ArtworkModule({ t, user, isAdmin, onBack, onGenerate }: {
  t: (k: string) => string; user: any; isAdmin: boolean;
  onBack: () => void; onGenerate: () => Promise<boolean>;
}) {
  ...
  const styles = [
    { id: 'cinematic', label: t('artwork.styleCinematic') },
    { id: 'anime', label: t('artwork.styleAnime') },
    { id: 'abstract', label: t('artwork.styleAbstract') },
    { id: 'vintage', label: t('artwork.styleVintage') },
    { id: 'minimal', label: t('artwork.styleMinimal') },
    { id: 'neon', label: t('artwork.styleNeon') },
  ];
      ...
      {result ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
          <div className="glass-card rounded-3xl overflow-hidden">
            <img src={result} alt="Generated artwork" className="w-full aspect-square object-cover" />
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => setResult(null)}
              className="flex-1 py-3 bg-white/5 text-white font-bold uppercase tracking-widest text-xs rounded-xl hover:bg-white/10 transition-all">
              {t('artwork.generateAnother')}
            </button>
          </div>
        </motion.div>
      ) : (
        <div className="space-y-6">
          <div className="glass-card rounded-3xl p-6 lg:p-8 space-y-5">
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('artwork.prompt')}</label>
            ...
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-3 block">{t('artwork.style')}</label>
              <div className="grid grid-cols-3 gap-2">
                {styles.map(s => (
                  <button key={s.id} onClick={() => setStyle(s.id)}
                    className={`py-2.5 px-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${style === s.id ? 'bg-turquoise text-black' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}>
                    {s.label}
                  </button>
                ))}
              </div>
          ...
          {generating && (
            <div className="glass-card rounded-2xl p-6 text-center">
              <div className="w-20 h-20 mx-auto mb-4 rounded-2xl bg-turquoise/10 animate-pulse" />
              <p className="text-white/40 text-xs">{t('artwork.processingImagen')}</p>
            </div>
          )}
        </div>
      )}
...
// ═══════════════════════════════════════════════════════════
// ─── BIO / PRESS KIT MODULE (Gemini-powered) ────────────
// ═══════════════════════════════════════════════════════════
function BioEPKModule({ t, user, isAdmin, onBack, onGenerate, lang }: {
  t: (k: string) => string; user: any; isAdmin: boolean;
  onBack: () => void; onGenerate: () => Promise<boolean>; lang: LanguageCode;
}) {
  const [artistName, setArtistName] = useState('');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const langNames: Record<string, string> = { en: 'English', fr: 'French', es: 'Spanish', it: 'Italian', de: 'German' };
      const prompt = `You are a professional music publicist. Write a compelling artist bio/press kit for an independent musician.

Artist: ${artistName}
Genre: ${genre}
Mood/Vibe: ${mood || 'not specified'}
Achievements: ${achievements || 'independent artist'}

Write in ${langNames[lang] || 'English'}. Include:
1. A short bio (3-4 sentences, punchy and professional)
2. A long bio (2 paragraphs, more detailed)
3. Press-ready one-liner
4. Suggested press contact template

Format with clear headers. Be authentic, avoid clichés.`;
  ...
  const copyBio = () => {
    if (generatedBio) navigator.clipboard.writeText(generatedBio);
  };
    ...
    <div className="max-w-3xl mx-auto p-4 lg:p-8">
      ...
      {generatedBio ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="glass-card rounded-3xl p-6 lg:p-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black uppercase tracking-widest text-turquoise">{t('epk.result')}</h3>
              <button onClick={copyBio} className="px-4 py-1.5 bg-white/5 text-white/60 text-[10px] font-bold uppercase tracking-widest rounded-lg hover:bg-white/10 transition-all flex items-center gap-1.5">
                <Copy className="w-3 h-3" /> {t('action.copy')}
              </button>
            </div>
            <div className="prose prose-invert prose-sm max-w-none whitespace-pre-wrap text-white/70 leading-relaxed">
              {generatedBio}
            </div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => { setGeneratedBio(null); }} className="flex-1 py-3 bg-white/5 text-white font-bold uppercase tracking-widest text-xs rounded-xl hover:bg-white/10 transition-all">
              {t('epk.regenerate')}
            </button>
          </div>
        </motion.div>
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-3xl p-6 lg:p-8 space-y-5">
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('epk.artistName')}</label>
              <input type="text" value={artistName} onChange={e => setArtistName(e.target.value)}
                className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('epk.genre')}</label>
                <input type="text" value={genre} onChange={e => setGenre(e.target.value)}
                  className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/15"
                  placeholder={t('epk.genrePlaceholder')} />
              </div>
              <div>
                <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('epk.mood')}</label>
            ...
            <div>
              <label className="text-[10px] text-white/40 font-bold uppercase tracking-widest mb-2 block">{t('epk.achievements')}</label>
          ...
          <button onClick={handleGenerate} disabled={generating || !artistName.trim() || !genre.trim()}
            className="w-full py-4 bg-turquoise text-black font-bold uppercase tracking-widest rounded-xl hover:brightness-110 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
            {generating ? <><Loader2 className="w-5 h-5 animate-spin" /> {t('epk.generating')}</> : <><Wand2 className="w-5 h-5" /> {t('epk.generate')} — 5 {t('dash.tokens')}</>}
          </button>
        </div>
      )}
    </div>
...
// ═══════════════════════════════════════════════════════════
// ─── MY ACCOUNT PAGE (profile, NOT pricing) ──────────────
// ═══════════════════════════════════════════════════════════
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 lg:p-10 space-y-8">
      ...
      <div className="glass-card rounded-3xl p-8 lg:p-10 text-center">
        ...
        <p className="text-6xl lg:text-7xl font-black text-gold">{isAdmin ? '∞' : user.points}</p>
        <p className="text-white/30 text-sm font-bold uppercase tracking-widest mt-2">{t('account.tokensRemaining')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          ...
          { label: t('account.memberSince'), value: user.createdAt?.toDate?.()?.toLocaleDateString?.() || '—', icon: Clock },
        ].map((item, i) => (
      ...
      <div className="glass-card rounded-3xl p-6 lg:p-8">
        <h3 className="text-sm font-black uppercase tracking-widest text-white/60 mb-4">{t('account.history')}</h3>
...
// ── Membership / Pricing (tarifs ONLY) ────────────────────
        ...
        {plans.map(plan => {
          ...
          return (
            <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <button disabled={isCurrent} className={`w-full py-3 rounded-xl font-bold uppercase tracking-widest text-xs transition-all ${isCurrent ? 'bg-white/5 text-white/30 cursor-default' : plan.id === 'artist' ? 'bg-turquoise text-black hover:brightness-110' : plan.id === 'pro' ? 'bg-purple-neon text-white hover:brightness-110' : 'bg-white/5 text-white hover:bg-white/10'}`}>
...
function ContactPage({ t }: { t: (k: string) => string }) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    console.log('[Contact]', { subject, message });
    setSending(false); setSent(true); setSubject(''); setMessage('');
  };
  return (
    <div className="max-w-2xl mx-auto p-6 lg:p-10">
      <h1 className="text-3xl font-black uppercase tracking-tight text-white mb-2">{t('contact.title')}</h1>
      <p className="text-white/40 text-sm mb-8">{t('contact.subtitle')}</p>
      {sent ? (
        ...
        <form onSubmit={handleSend} className="glass-card rounded-3xl p-6 lg:p-8 space-y-5">
          ...
          <button type="submit" disabled={sending || !subject.trim() || !message.trim()} className="w-full py-3 bg-turquoise text-black font-bold uppercase tracking-widest rounded-xl hover:brightness-110 disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <div className="space-y-3">
        {faqs.map((faq, i) => (
          <div key={i} className="glass-card rounded-2xl overflow-hidden">
            ...
            <AnimatePresence>{open === i && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}><p className="px-5 pb-5 text-white/50 text-sm leading-relaxed">{faq.a}</p></motion.div>)}</AnimatePresence>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 lg:p-10">
      ...
      <div className="space-y-4">
        {tutorials.map((tut, i) => (<div key={i} className="glass-card rounded-2xl p-6 flex items-start gap-5"><div className={`p-3 rounded-xl bg-white/5 ${tut.color}`}><tut.icon className="w-6 h-6" /></div><div><h3 className="text-base font-bold text-white">{tut.title}</h3><p className="text-white/40 text-sm mt-1">{tut.desc}</p></div></div>))}
        <div className="glass-card rounded-2xl p-6 flex items-center gap-5 opacity-50"><div className="p-3 rounded-xl bg-white/5 text-purple-neon"><Video className="w-6 h-6" /></div><p className="text-white/40 text-sm italic">{t('tutorials.comingSoon')}</p></div>
      </div>
    </div>
  );
}
...
function App() {
      ...
      if (u) {
        try {
          ...
          if (snap.exists()) {
            ...
            data = { email: u.email, displayName: u.displayName || '', avatar: u.photoURL || '', credits: 50, points: 50, isPro: false, plan: 'guest', createdAt: serverTimestamp() };
        ...
        } catch (err) { console.error("Auth error:", err); setUser(u); }
  ...
  const handleGenerate = async (moduleId: string): Promise<boolean> => {
    ...
    if (isAdmin) return true; // Admin bypass — no debit
  ...
  const handleLogout = async () => {
    await signOut(auth); setIsAdmin(false); setActiveModule('landing'); setIsMenuOpen(false); navigate('/');
  };

  const openAuth = (signUp: boolean) => {
    setShowAuthModal(true); setIsSignUp(signUp); setAuthError(null); setResetSent(false); setIsMenuOpen(false);
  };
  ...
  function renderModule() {
    ...
    switch (activeModule) {
      case 'smart-link':
        return <SmartLinkModule t={t} user={user} isAdmin={isAdmin} onBack={goHome} onGenerate={() => handleGenerate('smart-link')} />;
      case 'epk': case 'bio':
  ...
  // ═══════════════════════════════════════════════════════
  // ─── Sidebar (with Dashboard link at top) ─────────────
  // ═══════════════════════════════════════════════════════
      ...
      <aside className="hidden lg:flex flex-col w-20 xl:w-72 border-r border-white/5 bg-[#050505]/50 backdrop-blur-xl z-30">
        <div className="flex-1 py-6 px-3 space-y-2">
          ...
          <button onClick={goHome}
            className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all group ${activeModule === 'dashboard' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
          ...
          {/* Module links (skip dashboard since it's above) */}
          ...
          {/* Release Hub */}
          <button onClick={() => setActiveModule('release-hub')} className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all group ${activeModule === 'release-hub' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
          ...
          <div className="h-screen flex flex-col pt-[80px]">
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              ...
              <main className="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-br from-[#050505] to-[#0B0E14] relative pb-20 lg:pb-0">
                <Suspense fallback={<ModuleLoader />}><LandingPage t={t} onGetStarted={() => setActiveModule('dashboard')} /></Suspense>
          ...
          {isMenuOpen && (<>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="fixed top-0 left-0 h-full w-80 bg-[#0B0E14] border-r border-white/10 z-[151] p-8 shadow-2xl overflow-y-auto">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3"><img src="/logo-musaic.jpeg" alt="" className="w-9 h-9 rounded-xl" /><span className="text-lg font-black uppercase tracking-tight text-white">Menu</span></div>
              ...
              <div className="space-y-1">
                {/* Dashboard link in hamburger */}
                <button onClick={() => { goHome(); setIsMenuOpen(false); }} className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 transition-all group text-left">
                  <Home className="w-5 h-5 text-turquoise/60 group-hover:text-turquoise" />
                  <span className="text-xs font-bold uppercase tracking-widest text-white/60 group-hover:text-white">Artist Hub</span>
                </button>
                ...
                {([
                  ...
                  <button key={module} onClick={() => { setActiveModule(module as ModuleId); setIsMenuOpen(false); }} className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 transition-all group text-left">
                    <Icon className="w-5 h-5 text-white/40 group-hover:text-turquoise" /><span className="text-xs font-bold uppercase tracking-widest text-white/60 group-hover:text-white">{label}</span>
                  </button>
                ))}
                <div className="h-px bg-white/5 my-4" />
                {user ? (<>
                  <button onClick={handleLogout} className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 transition-all group text-left"><LogOut className="w-5 h-5 text-white/40" /><span className="text-xs font-bold uppercase tracking-widest text-white/60 group-hover:text-white">{t('nav.signOut')}</span></button>
