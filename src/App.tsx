...
import {
  Music, Shield, Zap, Lock, Mail, AlertCircle, Loader2,
...
// ─── Lazy-loaded modules (performance) ───────────────────
...
type ModuleId = 'dashboard' | 'smart-link' | 'bio' | 'epk' | 'video' | 'artwork' | 'strategy' | 'tutorials' | 'membership' | 'about' | 'faq' | 'privacy' | 'terms' | 'contact';

// ─── Nav items for sidebar + bottom bar ──────────────────
const NAV_ITEMS = [
...
// Bottom nav shows a subset (5 max for mobile)
...
// ─── Dashboard Component ─────────────────────────────────
function Dashboard({ t, onNavigate }: { t: (k: string) => string; onNavigate: (id: ModuleId) => void }) {
  const modules = [
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      <div className="mb-8">
        <h2 className="text-3xl lg:text-4xl font-black italic uppercase tracking-tighter text-white">Studio</h2>
        <p className="text-white/40 text-xs font-black italic uppercase tracking-[0.3em] mt-1">Choose your module</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:gap-6">
        {modules.map((mod, idx) => (
          <motion.button
            ...
            className="group relative flex flex-col items-center justify-center aspect-square bg-white/[0.02] border border-white/10 rounded-3xl overflow-hidden transition-all hover:bg-white/[0.05] hover:border-white/20 active:scale-[0.97]"
          >
            ...
            <h3 className="mt-4 text-sm lg:text-xl font-black italic uppercase tracking-tighter text-white group-hover:text-turquoise transition-colors text-center px-2">
              {mod.title}
            </h3>
          </motion.button>
        ))}
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <button
          ...
          className="flex items-center gap-3 p-4 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-white/5 transition-all group"
        >
          <Target className="w-5 h-5 text-orange-400" />
          <span className="text-xs font-black italic uppercase tracking-widest text-white/60 group-hover:text-white">Release Strategy</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-xs font-black italic uppercase tracking-widest text-white/60 group-hover:text-white">Tutorials</span>
        </button>
      </div>
    </div>
  );
...
function ProGate({ icon: Icon, t, color, onSignUp }: { icon: any; t: (k: string) => string; color: string; onSignUp: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-6">
      <div className={`w-20 h-20 ${color}/20 rounded-full flex items-center justify-center ring-1 ${color}/40`}>
        <Icon className={`w-10 h-10 ${color}`} />
      </div>
      <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white">{t('pro.feature')}</h2>
      <p className="text-gray-400 text-sm font-medium max-w-md">{t('pro.videoMessage')}</p>
      <button
        onClick={onSignUp}
        className="px-8 py-4 bg-white text-black font-black italic uppercase tracking-widest rounded-xl hover:bg-turquoise transition-all"
      >
        {t('pro.unlock')}
      </button>
    </div>
...
function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') setAuthError('ACCOUNT ALREADY EXISTS. PLEASE LOGIN.');
      else if (['auth/wrong-password', 'auth/user-not-found', 'auth/invalid-credential'].includes(error.code)) setAuthError('INVALID CREDENTIALS.');
      else if (error.code === 'auth/weak-password') setAuthError('PASSWORD TOO WEAK. USE 6+ CHARACTERS.');
      else setAuthError(`AUTH ERROR: ${error.message}`);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (['auth/popup-closed-by-user', 'auth/cancelled-popup-request'].includes(error.code)) setAuthError('GOOGLE AUTHENTICATION CANCELLED.');
      else setAuthError(`GOOGLE AUTH ERROR: ${error.message}`);
    } finally { setIsLoggingIn(false); }
  };
    ...
    return (
      <ErrorBoundary>
        <div className="min-h-screen bg-[#050505] text-white font-sans antialiased overflow-x-hidden">
          <Header
            user={null} isAdmin={false} isStarted={false}
  ...
  function renderModule() {
    switch (activeModule) {
      case 'dashboard':
        return <Dashboard t={t} onNavigate={setActiveModule} />;
      case 'smart-link':
      ...
      case 'bio':
        // Bio is now handled inside PressKitModule via a tab — redirect to epk
        return <PressKitModule lang={currentLang} aiPreferences={aiPreferences} />;
      case 'artwork':
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                {authError && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-red-400/80 text-[10px] font-medium uppercase tracking-wide">{authError}</p>
                  </motion.div>
                )}

                {resetSent && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                    className="bg-turquoise/10 border border-turquoise/50 p-4 rounded-xl text-turquoise text-[10px] font-medium uppercase tracking-wide">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black italic uppercase tracking-widest text-white/40 ml-4">{t('auth.email')}</label>
                    <div className="relative group">
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                        className="w-full bg-black/40 border border-white/10 rounded-xl py-4 pl-12 pr-4 focus:outline-none focus:border-turquoise/40 transition-all text-sm placeholder:text-white/10"
                        placeholder="name@example.com" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black italic uppercase tracking-widest text-white/40 ml-4">{t('auth.password')}</label>
                    <div className="relative group">
                ...
                <div className="space-y-4 pt-4">
                  <button type="submit" disabled={isLoggingIn}
                    className="w-full bg-white text-black font-black italic uppercase tracking-widest py-4 rounded-xl hover:bg-turquoise transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                    {isLoggingIn ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Authorizing...</span></> : (isSignUp ? t('auth.createAccount') : t('auth.signIn'))}
                  </button>

                  <div className="relative flex items-center py-2">
                    <div className="flex-grow border-t border-white/10" />
                    <span className="mx-4 text-white/20 text-[10px] font-black italic uppercase tracking-widest">{t('auth.orContinueWith')}</span>
                    <div className="flex-grow border-t border-white/10" />
                  </div>

                  <button type="button" onClick={handleGoogleLogin} disabled={isLoggingIn}
                    className="w-full bg-white/5 text-white border border-white/10 font-black italic uppercase tracking-widest py-4 rounded-xl hover:bg-white/10 transition-all disabled:opacity-50 flex items-center justify-center gap-3">
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-turquoise selection:text-black antialiased overflow-x-hidden">
        <Header
          user={user} isAdmin={isAdmin} isStarted={true}
          onLogin={() => openAuth(false)}
          onSignUp={() => openAuth(true)}
          onMenuOpen={() => setIsMenuOpen(true)}
          currentLang={currentLang}
        ...
        <div className="h-screen flex flex-col pt-[80px]">
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              ...
              <div className="flex-1 py-8 px-4 space-y-3">
                {NAV_ITEMS.map((item) => (
                  ...
                  >
                    ...
                    <span className="hidden xl:block text-xs font-black italic uppercase tracking-widest">{t(item.label)}</span>
                    {activeModule === item.id && (
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all group ${
                    activeModule === 'tutorials' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <BookOpen className={`w-6 h-6 ${activeModule === 'tutorials' ? 'text-turquoise' : 'group-hover:text-turquoise transition-colors'}`} />
                  <span className="hidden xl:block text-xs font-black italic uppercase tracking-widest">{t('nav.tutorials')}</span>
                </button>
                ...
                >
                  ...
                  <span className="hidden xl:block text-xs font-black italic uppercase tracking-widest">{t('nav.aiPersona')}</span>
                </button>
              </div>
              ...
              <div className="p-4 border-t border-white/5">
                <div className="flex items-center gap-3 px-4 py-3 bg-white/[0.02] rounded-2xl">
                  ...
                  <div className="hidden xl:block">
                    <p className="text-[10px] font-black italic uppercase tracking-widest text-white/40">Tokens</p>
                    <p className="text-lg font-black text-white">{user.points?.toLocaleString()}</p>
                  </div>
                  <span className="xl:hidden text-xs font-black text-gold">{user.points}</span>
                </div>
              </div>
            </aside>
            ...
            <main className="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-br from-[#050505] to-[#0B0E14] relative pb-20 lg:pb-0">
              ...
              {!['smart-link', 'dashboard'].includes(activeModule) && (
          ...
          <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#050505]/95 backdrop-blur-xl border-t border-white/10 safe-area-pb">
          ...
          {isMenuOpen && (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                onClick={() => setIsMenuOpen(false)} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60]" />
              <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
                className="fixed top-0 left-0 h-full w-80 bg-[#0B0E14] border-r border-white/10 z-[70] p-8 shadow-2xl">
                ...
                <div className="space-y-2">
                  {[
                    { icon: User, label: t('nav.myAccount'), action: () => { setActiveModule('membership'); setIsMenuOpen(false); } },
                    { icon: BookOpen, label: t('nav.tutorials'), action: () => { setActiveModule('tutorials'); setIsMenuOpen(false); } },
                    { icon: HelpCircle, label: t('nav.faq'), action: () => { setActiveModule('faq' as ModuleId); setIsMenuOpen(false); } },
                    { icon: Mail, label: t('nav.contact'), action: () => { setActiveModule('contact' as ModuleId); setIsMenuOpen(false); } },
                    { icon: Shield, label: t('nav.privacy'), action: () => { setActiveModule('privacy' as ModuleId); setIsMenuOpen(false); } },
                    { icon: FileText, label: t('nav.terms'), action: () => { setActiveModule('terms' as ModuleId); setIsMenuOpen(false); } },
                    ...
                    <button key={label} onClick={action}
                      ...
                      <span className="text-xs font-black italic uppercase tracking-widest text-white/60 group-hover:text-white">{label}</span>
                    </button>
                  ))}
                  ...
                  <button onClick={handleLogout}
                    ...
                    <span className="text-xs font-black italic uppercase tracking-widest text-white/60 group-hover:text-white">{t('nav.signOut')}</span>
                  </button>
                </div>
                ...
                <button className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-red-500/10 transition-all group text-left mt-8">
                  ...
                  <span className="text-xs font-black italic uppercase tracking-widest text-red-500/60 group-hover:text-red-500">{t('nav.deleteAccount')}</span>
                </button>
              </motion.div>
            </>
          )}
